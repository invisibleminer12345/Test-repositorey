...
This is the code repository of team 16932 Wildcats!
...
Knowledge about the FTC SDK is useful as well, and being proficient with Java is a good prerequisite.

## Getting Started

Coding in Java, no blocks here!
...
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;git clone https://github.com/WebberWildcats16932/Watergame.git</p>
...
## Building
Upon cloning the repo, open it in android studio, sync gradle, and then create a gradle configuration of type android application.

Make sure that this config is at the workspace root and name it TeamCode.

At this point, connect your computer to the robot using a usb-c to usb cable.

Upon doing so, as long as the control hub is powered, you should recieve a notification about a new device Control Hub v1.0, which should also show up in your Android Studio devices.

Select it in the device dropdown menu.

Navigate to cmd.exe on your computer and cd into C:\Users\[USER]\AppData\Local\Android\Sdk\platform-tools.

Connect to the robot wifi using netsh wlan connect 16932-RC.

Then, enter adb kill-server.

After a few seconds, enter adb tcpip 5555.

After the daemon has started, enter adb connect 192.168.43.1.

You should receive the message connected to 192.168.43.1:5555, if you do not, try restarting the robot.

Navigate back to android studio, and press the hammer / play button next to your configuration selection.

Congratulations, you have just uploaded code to the robot!

## KNOWN EXCEPTION EVENTS

FatalError: Can not build robot

Unfortunately, the only solve is to go into OnBlockJava and reset all of the onboard code files.
THIS TENDS TO HAPPEN when you reorganize packages in the repository.

RunTimeException: Can not find motor (Arm) [After you have already played with the robot]

Check wiring for both the expansion hub and the arm motor.
If no wire faults are found, this is likely because the arm motor overheated.

CompileError: Unknown Class R

You have incorrectly built to the robot. Try again and make sure your configurations are correct.
If it doesn't work, refer to the solution for FatalError: Can not build robot

BuildError: Changes are not tracked, unable determine incremental changes.

It is a known problem with android studio and cloudsharing with OneDrive.
Make sure the repository is in Keep on this Computer mode.
If it already is, disconnect from the robot wifi and instead connect to a local Wifi with connection, and then connect the robot to your computer by wire.
If it still shows the same error, toggle the mode on and off and try again.

## TODO (Atharva):

I HAVE UPDATED THE TEAMCODE PACKAGES TO INCLUDE PEDRO PATHING AND REORGANIZED PREVIOUS CODE.
Thus, you will most likely encounter a fatal error.
DO THIS WHILE THE BUILDERS ARE BUILDING.
